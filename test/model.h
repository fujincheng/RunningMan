...
private:
	...
	void loadModel() {
		...
		scaleFactor = scene_max.x - scene_min.x;
		scaleFactor = aisgl_max(scene_max.y - scene_min.y, scaleFactor);
		scaleFactor = aisgl_max(scene_max.z - scene_min.z, scaleFactor);
		scaleFactor = 1.f / scaleFactor;
		LoadGLTextures();
	}
	void LoadGLTextures() {
		ilInit();
		...
		string basepath = (string::npos == pos) ? "" : path.substr(0, pos + 1);
